...
const KelolaPelanggan = () => {
  // State sekarang dikosongkan karena data akan ditarik dari Laravel
  ...
  const API_URL = "http://localhost:8000/api/pelanggan";

  // AMBIL DATA DARI LARAVEL SAAT HALAMAN DIMUAT
  useEffect(() => {
    fetchData();
  }, []);
  ...
  // FILTER PENCARIAN (Berjalan otomatis setiap ketik keyword atau data berubah)
  useEffect(() => {
  ...
  // SIMPAN ATAU UPDATE DATA KE LARAVEL
  const handleSimpan = async (e) => {
    e.preventDefault();
    try {
      if (isEditing) {
        await axios.put(`${API_URL}/${formData.id}`, formData);
        Swal.fire({ title: "Diperbarui!", text: "Data pelanggan berhasil diubah.", icon: "success", timer: 1500, showConfirmButton: false });
      } else {
        await axios.post(API_URL, formData);
        Swal.fire({ title: "Tersimpan!", text: "Pelanggan baru berhasil ditambahkan.", icon: "success", timer: 1500, showConfirmButton: false });
      }
      fetchData(); // Refresh data dari database
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          Swal.fire({ title: "Terhapus!", text: "Data pelanggan berhasil dihapus.", icon: "success", confirmButtonColor: "#4f46e5" });
        } catch (error) {
              ...
              {filteredPelanggan.length === 0 ? (
                <tr>
                  <td colSpan="3" className="px-6 py-12 text-center text-slate-500 bg-slate-50/50 rounded-xl">
                    ...
                    Pelanggan tidak ditemukan atau belum ada data.
                  </td>
                </tr>
              ) : (
